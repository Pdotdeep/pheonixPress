...
// =============================================================================
	...
	public function replaceTableNamesWithAbstracts($tableName, $tableSql, $withDependecies = false)
	{
		...
		if (is_null($validCharRegEx))
		{
			$brokenPCRE     = @preg_match('/[0-9a-zA-Z$_\X]/u', 's') === false;
			$noUnicode      = @preg_match('/\p{L}/u', 'σ') !== 1;
			$validCharRegEx = $noUnicode ? '/[0-9a-zA-Z$_]/' : ($brokenPCRE ? '/[0-9a-zA-Z$_]|[\x{0080}-\x{FFFF}]/u' : '/[0-9a-zA-Z$_\X]/u');
		}
		...
		foreach ($tableNameMap as $fullName => $abstractName)
		{
			...
			else
			{
				...
				while (true)
				{
					...
					$prevIsTableChar = empty($previousChar) ? false : preg_match($validCharRegEx, $previousChar);
					$nextIsTableChar = empty($nextChar) ? false : preg_match($validCharRegEx, $nextChar);
